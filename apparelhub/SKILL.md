...
| **Channel performance** — which listings people SEE but do not buy, why a listing is underperforming, what is safe to archive, or whether a channel reports views at all | `references/channel-performance.md` covers the three `/agents/v1/analytics/channel/*` endpoints, the six listing states and the fix each implies, and the rule that a missing metric is never a zero |
