...
**Size charts are automatic for apparel — until they aren't.** When a product syncs to TikTok, a size chart is generated from the fulfillment provider's real per-size measurements (rendered to a table image), so you normally don't build or attach one. It's best-effort: if the provider publishes no measurements for that garment, the chart is omitted and the listing still syncs.

That omission is not free. US apparel with no size chart is graded down by the channel, and only one of the three fulfillment providers publishes measurements — so a listing on a blank from either of the others gets no chart at all. If a listing is flagged for it, call `import_size_measurements` and set `size_chart_measurements`; see `references/listing-attributes.md` § Size charts. **Never type in measurements you have not been given** — a wrong size chart is worse than a missing one.
