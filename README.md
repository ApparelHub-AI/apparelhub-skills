...
|---|---|---|
| 2.8 | 2026-06-30 | **Workspace discovery route (epic apparelhub-ai#416).** Documents the new `GET /agents/v1/workspaces` route — the piece v2.6 was missing. v2.6 told the agent "there's no agent endpoint that lists an account's workspaces," so an agent asked to scope to a workspace by name (e.g. "the Crystal Riley client") had no way to resolve the name to a `uuid` and choked. Now `GET /agents/v1/workspaces` returns `{workspaces:[{uuid,name,is_default}], active_workspace, key_scope}`; the agent matches the user's name to a `uuid`, then scopes with `?workspace=<uuid>`. A workspace-pinned key lists only its own workspace. `references/workspaces.md` (§2) now leads with the discovery route, and `SKILL.md` (§6) + `references/api-contract.md` (§4b) reference it. Docs only — no script, settings, or installer changes. |
