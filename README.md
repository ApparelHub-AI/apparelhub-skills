...
└── apparelhub/
    ...
    ├── scripts/
    │   └── make_transparent.py               # Packaged Phase 2 keying tool (whitelisted by path)
    ├── references/
...
By default Claude Code asks for confirmation before every `curl`, `python3`, and `jq` invocation the skill makes — which adds up fast across the 7-phase product-creation pipeline. The repo ships a `settings.recommended.json` that allowlists the exact patterns the skill uses (and nothing broader):
