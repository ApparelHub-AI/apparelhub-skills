...
| 2.31 | 2026-07-29 | **All-over print: read `warnings`, and check the BACK render** (`references/all-over-print.md` §10). A fill build could report `print_style: fill`, every placement covered, full-bleed geometry and an empty `warnings` array while shipping a patterned front with a **blank back and blank sleeves** — and those blank files reached the fulfillment provider. The platform now checks composed pixels and reports a single-colour print file or one file reused across different print areas; treat either as ship-blocking. Also: never pad/letterbox a design destined for all-over print — the flat bars print as an unpatterned band at the hem, and cover-fit cannot remove them once the padded frame matches the area aspect. Docs only. |
